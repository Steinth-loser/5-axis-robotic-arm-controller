...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
	//stepper_init(&my_stepper2, &htim2);
	//stepper_set_direction(&my_stepper2, true);
	//stepper_set_speed(&my_stepper2, 30);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	// When data comes from uart, handle it
	if (huart->Instance == USART1) {
		Message_t rx_msg;
		memcpy(rx_msg.data_pack, rx_buffer, 28);
		osMessageQueuePut(messageQueueHandle, &rx_msg, 0, 0);
	}
	HAL_UART_Receive_IT(&huart1, rx_buffer, 28);
}
/* USER CODE END 4 */
